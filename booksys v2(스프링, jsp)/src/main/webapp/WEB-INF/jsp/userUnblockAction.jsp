...
<%@ include file="../header/loginCheck.jsp" %>
	<%
		try {
			...
			for(String v:value){	// 선택된 고객 차단 해제
				...
				if(grade<6 || grade==0) continue;					// 차단 안된 고객이거나 관리자일 경우 패스
				
				url=new URL(api_url+"api/customer/update");			// API로 고객 정보 업데이트
				conn=(HttpURLConnection)url.openConnection();
				conn.setRequestProperty("Content-Type", "application/json");
				conn.setDoOutput(true);
				String json="{\"userId\":\""+v+"\",\"grade\":"+(grade-5)+"}";	// json 형태로 전송
			...
			script = response.getWriter();
			script.println("<script>");
			script.println("alert('차단 해제되었습니다.')");
			script.println("location.href='userManager'");
			script.println("</script>");
		} catch(Exception e){	// 1개 이상 선택 안 할 경우 접근 불가
			script = response.getWriter();
			script.println("<script>");
